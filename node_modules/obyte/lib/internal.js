...
function getNakedUnit(objUnit) {
  ...
  if (objUnit.version === _constants.VERSION_WITHOUT_TIMESTAMP) delete objNakedUnit.timestamp;
...
function getHeadersSize(objUnit) {
  ...
  if (objUnit.version === _constants.VERSION_WITHOUT_TIMESTAMP) delete objHeader.timestamp;
...
function getUnitHashToSign(objUnit) {
  ...
  }var sourceString = objUnit.version === _constants.VERSION_WITHOUT_TIMESTAMP ? getSourceString(objNakedUnit) : getJsonSourceString(objNakedUnit);
  return (0, _createHash2.default)('sha256').update(sourceString, 'utf8').digest();
}

function getUnitContentHash(objUnit) {
  return getBase64Hash(getNakedUnit(objUnit), objUnit.version !== _constants.VERSION_WITHOUT_TIMESTAMP);
}

function getUnitHash(objUnit) {
  var bVersion2 = objUnit.version !== _constants.VERSION_WITHOUT_TIMESTAMP;
