...
var _JsonDBConfig = require("./lib/JsonDBConfig");
...
class JsonDB {
  ...
  constructor(filename, saveOnPush = true, humanReadable = false, separator = '/') {
    ...
    if (filename instanceof _JsonDBConfig.Config) {
      this.config = filename;
    } else {
      this.config = new _JsonDBConfig.Config(filename, saveOnPush, humanReadable, separator);
    }
  ...
  processDataPath(dataPath) {
    ...
    dataPath = (0, _Utils.removeTrailingChar)(dataPath, this.config.separator);
    ...
    const recursiveProcessDataPath = (data, index) => {
      ...
      function findData(isArray = false) {
        if (data.hasOwnProperty(property)) {
          ...
          throw new _Errors.DataError(`Can't find dataPath: ${thisDb.config.separator}${dataPath.join(thisDb.config.separator)}. Stopped at ${property}`, 5);
        }
      }
      ...
      if (arrayInfo) {
        ...
        if (!Array.isArray(data)) {
          throw new _Errors.DataError(`DataPath: ${thisDb.config.separator}${dataPath.join(thisDb.config.separator)}. ${property} is not an array.`, 11);
        }
        ...
        if (!arrayInfo.append && data.hasOwnProperty(arrayIndex)) {
          ...
          throw new _Errors.DataError(`DataPath: ${thisDb.config.separator}${dataPath.join(thisDb.config.separator)}. . Can't find index ${arrayInfo.index} in array ${property}`, 10);
        }
      } else {
        findData();
      }
  ...
  save(force) {
    ...
    try {
      ...
      const error = new _Errors.DatabaseError("Can't save the database", 2, err);
      throw error;
    }
  }
